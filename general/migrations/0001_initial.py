# -*- coding: utf-8 -*-
# Generated by Django 1.11.15 on 2021-08-28 16:27
from __future__ import unicode_literals

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='BaseGame',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('data_source', models.CharField(choices=[('DraftKings', 'DraftKings'), ('FanDuel', 'FanDuel'), ('Yahoo', 'Yahoo')], default='FanDuel', max_length=30)),
                ('time', models.TimeField()),
                ('visit_team', models.CharField(max_length=20)),
                ('home_team', models.CharField(max_length=20)),
                ('visit_score', models.CharField(blank=True, max_length=50, null=True)),
                ('home_score', models.CharField(blank=True, max_length=50, null=True)),
                ('ou', models.FloatField(default=0)),
                ('ml', models.CharField(max_length=20)),
                ('updated_at', models.DateTimeField(auto_now=True)),
            ],
        ),
        migrations.CreateModel(
            name='BasePlayer',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('data_source', models.CharField(choices=[('DraftKings', 'DraftKings'), ('FanDuel', 'FanDuel'), ('Yahoo', 'Yahoo')], default='FanDuel', max_length=30)),
                ('uid', models.IntegerField()),
                ('first_name', models.CharField(max_length=50)),
                ('last_name', models.CharField(max_length=50)),
                ('avatar', models.CharField(default='/static/img/nba.ico', max_length=250)),
                ('team', models.CharField(max_length=50)),
                ('injury', models.CharField(blank=True, default='', max_length=250)),
                ('handedness', models.CharField(blank=True, max_length=5, null=True)),
                ('opp_pitcher_id', models.PositiveIntegerField(blank=True, null=True)),
                ('start', models.CharField(blank=True, max_length=5, null=True, verbose_name='Order')),
                ('start_status', models.CharField(blank=True, max_length=5, null=True, verbose_name='Confirm')),
                ('updated_at', models.DateTimeField(auto_now=True)),
            ],
        ),
        migrations.CreateModel(
            name='FavPlayer',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
            ],
        ),
        migrations.CreateModel(
            name='Game',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('home_team', models.CharField(max_length=20)),
                ('visit_team', models.CharField(max_length=20)),
                ('time', models.CharField(blank=True, default='', max_length=20, null=True)),
                ('home_score', models.CharField(blank=True, max_length=50, null=True)),
                ('visit_score', models.CharField(blank=True, max_length=50, null=True)),
                ('ou', models.FloatField(default=0)),
                ('ml', models.CharField(blank=True, default='', max_length=20, null=True)),
                ('display', models.BooleanField(default=True)),
                ('updated_at', models.DateTimeField(auto_now=True)),
            ],
        ),
        migrations.CreateModel(
            name='Player',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('rid', models.CharField(max_length=100)),
                ('uid', models.IntegerField(default=-1)),
                ('first_name', models.CharField(max_length=50)),
                ('last_name', models.CharField(max_length=50)),
                ('avatar', models.CharField(default='/static/img/nba.ico', max_length=250)),
                ('injury', models.CharField(blank=True, default='', max_length=250, null=True)),
                ('opponent', models.CharField(blank=True, default='', max_length=50, null=True)),
                ('position', models.CharField(max_length=50)),
                ('actual_position', models.CharField(max_length=50)),
                ('proj_points', models.FloatField()),
                ('proj_delta', models.FloatField(default=0)),
                ('salary', models.IntegerField(default=0)),
                ('team', models.CharField(max_length=50)),
                ('play_today', models.BooleanField(default=False)),
                ('opp_pitcher_id', models.PositiveIntegerField(blank=True, null=True)),
                ('handedness', models.CharField(blank=True, default='', max_length=5, null=True)),
                ('start', models.CharField(blank=True, default='', max_length=5, null=True)),
                ('start_status', models.BooleanField(default=False)),
                ('updated_at', models.DateTimeField(auto_now=True)),
            ],
        ),
        migrations.CreateModel(
            name='Slate',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('data_source', models.CharField(choices=[('DraftKings', 'DraftKings'), ('FanDuel', 'FanDuel'), ('Yahoo', 'Yahoo')], max_length=30)),
                ('name', models.CharField(max_length=120)),
            ],
        ),
        migrations.AddField(
            model_name='player',
            name='slate',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='players', to='general.Slate'),
        ),
        migrations.AddField(
            model_name='game',
            name='slate',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='games', to='general.Slate'),
        ),
        migrations.AddField(
            model_name='favplayer',
            name='player',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='general.Player'),
        ),
    ]
